...
class TestStateRepositoryInitialize:
    def test_creates_row(self, state_repo):
    ...
    def test_row_readable_after_init(self, state_repo):
        state_repo.initialize(Decimal("500.00"))
        loaded = state_repo.load()
        assert loaded is not None
        assert loaded.virtual_balance_free == Decimal("500.00")
        assert loaded.cycle_status == CycleStatus.IDLE

    def test_duplicate_initialize_raises(self, state_repo):
        state_repo.initialize(Decimal("100.00"))
        with pytest.raises(Exception):  # psycopg2 IntegrityError (PK conflict)
            state_repo.initialize(Decimal("200.00"))
...
class TestStateRepositoryLoad:
    def test_returns_none_when_missing(self, state_repo):
        result = state_repo.load()
        assert result is None

    def test_loads_initialized_state(self, state_repo):
        state_repo.initialize(Decimal("750.00"))
        loaded = state_repo.load()
        assert loaded is not None
    ...
    def test_active_dca_order_ids_is_tuple(self, state_repo):
        state_repo.initialize(Decimal("100.00"))
        loaded = state_repo.load()
...
class TestStateRepositorySave:
    def test_save_updates_fields(self, state_repo):
        state = state_repo.initialize(Decimal("1000.00"))
    ...
    def test_save_increments_version(self, state_repo):
        state = state_repo.initialize(Decimal("1000.00"))
    ...
    def test_save_preserves_dca_order_ids(self, state_repo):
        state = state_repo.initialize(Decimal("1000.00"))
        new_state = state.with_updates(
    ...
    def test_save_numeric_precision(self, state_repo):
        state = state_repo.initialize(Decimal("1000.00"))
    ...
    def test_version_conflict_raises(self, state_repo):
    ...
    def test_save_clears_nullable_fields(self, state_repo):
        state = state_repo.initialize(Decimal("1000.00"))
...
class TestStateRepositoryForUpdate:
    def test_load_for_update_returns_state(self, state_repo):
        state_repo.initialize(Decimal("1000.00"))
        loaded = state_repo.load(for_update=True)
        assert loaded is not None
        assert loaded.cycle_status == CycleStatus.IDLE

    def test_load_for_update_missing_returns_none(self, state_repo):
        result = state_repo.load(for_update=True)
        assert result is None
...
class TestRegistryRepositoryUpsert:
    def test_insert_on_first_call(self, registry_repo):
        registry_repo.upsert(status=OperationalStatus.STARTING)
        loaded = registry_repo.load()
        assert loaded is not None
        assert loaded.operational_status == OperationalStatus.STARTING

    def test_update_on_second_call(self, registry_repo):
        registry_repo.upsert(status=OperationalStatus.STARTING)
        registry_repo.upsert(status=OperationalStatus.RUNNING)
        loaded = registry_repo.load()
        assert loaded.operational_status == OperationalStatus.RUNNING

    def test_started_at_preserved_on_update(self, registry_repo):
        started = datetime(2026, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        registry_repo.upsert(status=OperationalStatus.STARTING, started_at=started)
        # Update status without started_at — should keep original value
        registry_repo.upsert(status=OperationalStatus.RUNNING)
        loaded = registry_repo.load()
        assert loaded.started_at is not None

    def test_pid_saved(self, registry_repo):
        registry_repo.upsert(status=OperationalStatus.RUNNING, pid=99999)
        loaded = registry_repo.load()
        assert loaded.pid == 99999

    def test_error_message_saved(self, registry_repo):
...
class TestRegistryRepositoryLoad:
    def test_returns_none_when_missing(self, registry_repo):
        result = registry_repo.load()
        assert result is None

    def test_returns_registry_after_upsert(self, registry_repo):
        registry_repo.upsert(status=OperationalStatus.STOPPED)
        loaded = registry_repo.load()
        assert loaded is not None
        assert loaded.user_id == "test_user"
        assert loaded.bot_id == "test_bot"
...
class TestRegistryRepositoryHeartbeat:
    def test_heartbeat_updates_timestamp(self, registry_repo):
        registry_repo.upsert(status=OperationalStatus.RUNNING)
        registry_repo.update_heartbeat()
        loaded = registry_repo.load()
        assert loaded.last_heartbeat is not None
        assert loaded.operational_status == OperationalStatus.RUNNING

    def test_heartbeat_on_missing_row_is_noop(self, registry_repo):
...
class TestRegistryRepositoryStatusMethods:
    def test_mark_stopped(self, registry_repo):
        registry_repo.upsert(status=OperationalStatus.RUNNING)
        registry_repo.mark_stopped()
        loaded = registry_repo.load()
        assert loaded.operational_status == OperationalStatus.STOPPED
        assert loaded.stopped_at is not None

    def test_mark_error(self, registry_repo):
        registry_repo.upsert(status=OperationalStatus.RUNNING)
        registry_repo.mark_error("Critical failure in OrderManager")
        loaded = registry_repo.load()
        assert loaded.operational_status == OperationalStatus.ERROR
