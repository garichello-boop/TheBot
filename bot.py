...
Предположения об интерфейсах П6 (сверить при интеграции):
  - StateRecovery.startup(user_id, bot_id, broker) → BotState
  - StateRepository(db_pool) → repo
  - RegistryRepository(db_pool) → repo
  - StateManager(db_pool, emitter) → manager

Предположения об интерфейсах П5:
  - ConfigRepository(db_pool) → repo
  - ConfigWatcher(repo, user_id, bot_id) → watcher
  - ConfigWatcher.create_snapshot() → CycleSnapshot
  - ConfigWatcher.set_close_only(user_id, bot_id) → None
  - ConfigWatcher.get_config() → BotConfig

Предположения об интерфейсах П3:
  - setup_observability(settings, bot_id, ticker, tg_token, tg_chat_id) → EventEmitter

Предположения об интерфейсах П2:
  - ProviderFactory.create(settings.market) → MarketDataProvider

Предположения об интерфейсах П4:
  - BrokerFactory.create(settings.broker, token=...) → IBroker
"""
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    # 9. State (П6): registry, repository, manager
    # ------------------------------------------------------------------
    try:
